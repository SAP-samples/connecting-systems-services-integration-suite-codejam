...
- *On Stack Extensibility*: Exclusively available for SAP S/4HANA Cloud and it allows you to create development projects in an SAP S/4HANA Cloud system.
- *Process Blueprints*: A process blueprint refers to a set of activities that have to be performed to complete an end-to-end business scenario.
- *Build*:
- *VPUC*: Validated Partner Use Cases are service provider packages that provide assets to help you maximize the return on investment on your SAP Business Technology Platform venture.
- *Analytics*: Analytical content available for the product, for example, SAP Analytics Cloud business content and SAP Datasphere business content
- *Transformation*: Explore business transformation journeys, which align with the SAP Activate methodology and are tailored to your industry and scenario.
...
🧭 Take some time to explore what's documented on the [Business Partner API - Overview page](https://hub.sap.com/api/API_BUSINESS_PARTNER/overview). You will be able to find lots of valuable information here, e.g. attributes, authentication methods, API specifications, and configuration details. In [exercise 06](../06-expose-integration-flow-api-management/README.md) we will explore API specifications further.
...
- [Explore the SAP Business Accelerator Hub](https://hub.sap.com/)
