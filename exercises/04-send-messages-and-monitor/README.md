...
- Manage Security: We can manage the security artefacts deployed in our tenant. If you faced the [PHIX certificate issue](../../troubleshooting.md#http-500-internal-server-error---pkix-path-building-failed-unable-to-find-valid-certification-path-to-requested-target) described on the troubleshooting page, you will be familiar with the `Keystore` tile. In a future exercise, we will deploy additional authentication credentials to our tenant through the `Security Material` tile.
...
Continue to 👉 [Exercise 05 - Retrieve Business Partner dependant's information](../05-retrieve-bp-dependants/)
